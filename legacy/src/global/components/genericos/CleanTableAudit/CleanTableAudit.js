...
import { ContainerTableGlobal } from "./localStyle";

const CleanTableAudit = ({ config, onChange, filtroBuscador }) => {
  //#region ------ useStates --------
  const [global, setGlobal] = useState({
    data: config.data, // lista de datos primaria
    dataFiltered: config.data, // lista filtrada
    dataSelected: [], // data seleccionada por usuario // caso aparte
    filter: [
      {
        column: "original",
        data: config.data,
    ...
    customSeguimiento: config.customSeguimiento,
  });
  ...
  // useEffect(() => {
  //   console.log("Es el filtro pepe ",global.filter);
  // },[global.filter])
  ...
  return (
    <ContainerTableGlobal width={config.width} widthQuery={config.widthQuery}>
      ...
      {config.footer ? (
        <Footer global={global} setGlobal={setGlobal} config={config} />
      ) : (
        ""
      )}
...
export default CleanTableAudit;
