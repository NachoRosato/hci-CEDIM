...
const DropdownV2 = ({ config, onClick }) => {
  //#region  ----------- constants ---------------
  ...
  const orderFilterData = () => {
    if (
      ...
      config?.orderCustom === undefined
    ) {
      return config?.data.sort((x, y) =>
  ...
  const [filteredData, setFilteredData] = useState(orderFilterData);
      ...
      if (config?.regex) {
        if (config?.regex.test(value)) {
          ...
          if (!config?.buscarPorDefault) {
            ...
            filtered = orderListaStartsWith(config?.data, value, "");
          }
          setFilteredData(filtered);
          if (config?.masBuscados) {
            
            const sortedFiltered = [...filtered].sort(
              (a, b) => b.cantBusquedas - a.cantBusquedas
            );

            let favs = sortedFiltered.filter((item) => item.cantBusquedas > 0);
            let otros = sortedFiltered.filter(
              (item) => item.cantBusquedas <= 0
            );

            let primerosFav = favs.slice(0, 5);

            let idsPrimeros = primerosFav.map((item) => item.id);

            let restoFavs = favs.filter(
              (item) => !idsPrimeros.includes(item.id)
            );

            setFavData(primerosFav);
            if (primerosFav.length > 0) {
              const turnosCompletos = primerosFav.concat(
                restoFavs.concat(otros)
              );
              setOtrosData(restoFavs.concat(otros));
              setFilteredData(turnosCompletos);
            } else {
              setOtrosData(otros);
              setFilteredData(otros);
            }
          }
        }
      } else {
        ...
        if (config?.masBuscados) {
          ...
          let favs = sortedFiltered.filter((item) => item.cantBusquedas > 0);
          let otros = sortedFiltered.filter((item) => item.cantBusquedas <= 0);
  ...
  return (
    <DropdownV2Container width={config?.width}>
      {config?.header && (
        <DropdownV2Header>
